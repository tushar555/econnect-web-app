...
@Component({
  selector: "app-admin-employee-config",
  templateUrl: "./admin-employeeFlexi-config.component.html",
  styleUrls: ["./admin-employeeFlexi-config.component.scss"]
})
export class AdminEmployeeFlexiConfigComponent implements OnInit {
  ...
  constructor(
    ...
    console.dir(this._services.AdminEmpFlexiConfig);
    if(this._services.AdminEmpFlexiConfig){
      this.getListData=this._services.AdminEmpFlexiConfig;
    }else{
  ...
  onItemSelectGrade(GradeCache) {
    ...
    console.dir(this.DateQuater, "DateQuater");
  ...
  getCompanyGradeName(CompName) {
    ...
    this._services.getUserDetail().then((data: any) => {
      ...
      this._services.postService(url, param).then((data: any) => {
        ...
        var flags = [],
          output = [],
          l = this.dropdownListGrade.length,
          i;
        for (i = 0; i < l; i++) {
  ...
  getFlexiDetails() {
    this._services.getUserDetail().then((data: any) => {
      ...
      this._services.postService(url, param).then((data: any) => {
        this.flexiDetails = data.FlexiMappingComponents;
        this.DateQuater = data.years[0];
        console.dir(this.DateQuater);
        this.RuleId = this.flexiDetails[0].RuleId;
        this.selectedItemsEmp = data.EmployeeIdExclude
          ? data.EmployeeIdExclude
          : [];
  ...
  submitFlexiDeclaration() {
    this._services.getUserDetail().then((data: any) => {
      ...
      this._services.postService(url, param).then(
        (data: any) => {
          console.log(data);
          // this.getFlexiDetails();
          this.spinnerService.hide();
  ...
  calculateFlexiDeclaration(limit, i) {
    ...
    this.flexiDetails.forEach(el => {
      if (el.FlexiId !== 9 && el.FlexiId !== 10) {
        ...
        if (!isNaN(el.DeclaredAmount)) this.DeclaredAmount += el.DeclaredAmount;
      }
    });
